...
def test_binomial_tree_addition(mpi_env):
    ...
    local_val = rank + 1
    result = binomial_tree_scan(local_val, add_op)
...
def test_binomial_tree_multiplication(mpi_env):
    comm, rank, size = mpi_env
    ...
    local_val = rank + 1
    result = binomial_tree_scan(local_val, mul_op)
...
def test_binomial_tree_matrix_multiplication(mpi_env):
    comm, rank, size = mpi_env
    ...
    result = binomial_tree_scan(local_val, matmul_op)
...
def test_binomial_tree_matrix_addition(mpi_env):
    comm, rank, size = mpi_env
...
def test_binomial_tree_edge_case_single_process(mpi_env):
    comm, rank, size = mpi_env
...
def test_binomial_tree_edge_case_zero_value(mpi_env):
    comm, rank, size = mpi_env
